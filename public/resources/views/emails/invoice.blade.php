        ...
        <div class="inner-container">
            @if (isset($setting->logo_path))
                <table border="0" cellpadding="0" cellspacing="0">
                    <tbody>
                        <tr>
                            <td class="header-top" align="center"><a href="{{ route('home') }}" target="_blank"><img
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td align="right" width="40%">
                                ...
                                <p>{{ __('email.invoice_date') }}:
                                    {{ date('d M Y', strtotime($data['invoice']->invoice_date)) }}</p>
                                @if (isset($data['invoice']->due_date))
                                    <p>{{ __('email.due_date') }}:
                                        {{ date('d M Y', strtotime($data['invoice']->due_date)) }}</p>
                                @endif
                        ...
                        @if (isset($data['message']))
                            @php
                                ...
                                $replace = [
                                    '<b>' . $data['invoice']->name . '</b>',
                                    '<b>' . $data['invoice']->email . '</b>',
                                    '<b>' . $data['invoice']->phone . '</b>',
                                    '<b>' . $data['invoice']->address . '</b>',
                                    '<b>' . $data['invoice']->city . '</b>',
                                    '<b>' . $data['invoice']->company . '</b>',
                                    '<b>' . $services_list . '</b>',
                                ];
                            ...
                            <tr>
                                <td class="full-width">
                                    {!! strip_tags(
                                        $message_body,
                                        '<p><a><b><i><u><strong><br><table><tr><th><td><ul><ol><li><h1><h2><h3><h4><h5><h6><del><ins><sup><sub><pre>',
                                    ) !!}
                                </td>
                            </tr>
                        @endif
                                        ...
                                        <tbody>
                                            @if (isset($data['invoice']->service_charge))
                                                <tr class="total">
                                                    ...
                                                    <td align="right"> {{ $data['invoice']->service_charge }}
                                                        {{ __('common.currency') }}</td>
                                                </tr>
                                            @endif
                                            @if (isset($data['invoice']->tax))
                                                <tr class="total">
                                                    <td width="80%">{{ __('email.tax') }}</td>
                                                    <td align="right"> {{ $data['invoice']->tax }}
                                                        {{ __('common.currency') }}</td>
                                                </tr>
                                            @endif
                                            @if (isset($data['invoice']->shipping))
                                                <tr class="total">
                                                    ...
                                                    <td align="right"> {{ $data['invoice']->shipping }}
                                                        {{ __('common.currency') }}</td>
                                                </tr>
                                            @endif
                                            <tr class="total">
                                                <td width="80%">{{ __('email.total_amount') }}</td>
                                                <td align="right"> {{ $data['invoice']->total_amount }}
                                                    {{ __('common.currency') }}</td>
                                            </tr>
                                            @if (isset($data['invoice']->discount_amount))
                                                <tr class="total">
                                                    <td width="80%">{{ __('email.discount_amount') }}</td>
                                                    <td align="right"> - {{ $data['invoice']->discount_amount }}
                                                        {{ __('common.currency') }}</td>
                                                </tr>
                                            @endif
                                            <tr class="total">
                                                <td width="80%">{{ __('email.payable_amount') }}</td>
                                                <td align="right"> = {{ $data['invoice']->invoice_amount }}
                                                    {{ __('common.currency') }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                            ...
                            <td colspan="2">
                                <center>
                                    @if (is_file('uploads/invoice/' . $data['invoice']->attach))
                                        <a href="{{ asset('uploads/invoice/' . $data['invoice']->attach) }}"
                                            class="attach-btn" target="_blank"
                                            download>{{ __('email.attach_btn') }}</a>
                                    @endif
                                    <a href="{{ route('make.payment', $data['invoice']->id) }}"
                                        class="payment-btn">{{ __('email.pay_btn') }}</a>
                                </center>
                                <b>- {{ __('email.thanks') }}</b>
                            </td>
                        </tr>
